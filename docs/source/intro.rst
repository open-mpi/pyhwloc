...
.. note::
   ...
   Currently, the Python interface is built upon the top-of-tree hwloc (3.x series), which
   has an updated and incompatible XML schema. As a result, one cannot load XML files
   generated by hwloc 2.x or earlier with PyHwloc.
